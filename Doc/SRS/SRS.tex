...
We are assuming that the user has an Android APK 23 package or iOS 8.0 or greater as well as enough memory on their phone to download and install the app. We also assume that the user is familiar with a menu page navigation style for the application to navigate from page to page. % I only really like the last assumption, App store will handle matching version requirements. Plus, you have already said you require them to have that version, no need to assume as they would all have it.
...
There are a couple of use case scenarios that would make the application user-friendly. These include the following. % include any ones not in the software in a likely changes section
\begin{itemize}
...
\subsection{Functional Requirements}
   \begin{enumerate}
     \item The application will allow the user to input and delete transactions on a past or current day % be more atomic!
     \item The application will allow the user to view previous transactions and view the trend of their previous transactions % atomic
     \item  The application will allow the user to edit previous transactions
     \item The application will record all inputs by the user on a database % better to say user's data persists over sessions, more abstract
   \end{enumerate}
...
\subsection{Look and Feel Requirements}
   \begin{enumerate}
   \item The application should have a clean user interface % how to measure and validate?
   \begin{itemize}
   ...
   \begin{itemize}
     \item The application should run smoothly, with minimal wait/loading times. % what is minimal?
   \end{itemize}
\end{enumerate}
\subsection{Usability and Humanity Requirements}
   ...
   \begin{itemize}
     \item 90\% of users should be able to find all functions of the application without help. The layout will be easy to understand, which will allow the user to navigate through the application. % nice!!!!!
   \end{itemize}
 \end{enumerate}
\subsection{Performance Requirements}
All recent transactions or edits of previous transactions should be reflected on the graph with no delay % this is literally impossible

\subsection{Operational and Environmental Requirements}
   \begin{enumerate}
     \item The application should not take up too much storage space on a phone % how much is too much?
     \item The application works on both Android and iOS
   \end{enumerate}

\subsection{Maintainability and Support Requirements}
If the application crashes, none of the data should be lost % tough to test, think more of ACID transactions when making this stuff
...
Here are the following off-the-shelf solutions were are going to be using:  % refer more to the problem above
   \begin{enumerate}
...
\subsection{Risks}
Risks involving that application, would involve the database. The database can crash, and this could lead to a possibility of losing data. Another issue would be not having access to the database, which leads to losing data that was entered through the application. % is this an external db? cause that conflicts with a requirement you set...
\subsection{Costs}
